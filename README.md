...
`rasch` implements Rasch Measurement Theory in R. It provides a common set of
models and diagnostics for constructing measurement scales and examining
...
- **Sufficiency:** the relevant score contains the information required for
  person measurement.
- **Invariance:** comparisons between persons and items should remain stable
...
estimates, but also the fit, dimensionality, local dependence and differential
item functioning analyses needed to determine whether those estimates support
an invariant measurement scale.
...
measurement based on weighted likelihood. Anchored estimation is available for
equating. Planned missingness and linked designs are supported where the
observed response graph identifies a common scale. Informative response
missingness is not made ignorable by the estimator and should be examined
separately.
