...
For an item with ordered scores $x=0,\ldots,m_i$, `rasch()` estimates the
...
where $\theta_n$ is the location of person $n$ and $\delta_{ix}$ is the
threshold between categories $x-1$ and $x$; dichotomous items have one
threshold, and the rating scale model holds a common threshold structure
across the items. The total score is sufficient for $\theta_n$, so
`rasch()` estimates the item parameters by pairwise conditioning
...
follow the adjacent-category form above with $\beta_a-\beta_b$ in place
of $\theta_n$ and a single symmetric threshold set shared by all pairs.
