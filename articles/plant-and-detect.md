...
| Person fit bootstrap | 240 persons x 12 dichotomous items, B = 99; four-category PCM at 500 persons, B = 199 | score-conditional fit-residual marginal error was 2.4% for the dichotomous design and 4.08% in the PCM top-up. The earlier familywise rows used in-sample maxT standardisation and are superseded; the current leave-one-out calculation is covered by the exchangeability study and the model studies are retained as the rerun specification |
| Paired-comparison fit bootstrap | 6 objects x 20 judges; dichotomous and four-category responses | total-test, pair-family and marginal-judge results remain valid because they do not use estimated maxT standardisation. Earlier object- and judge-family rows are superseded by the leave-one-out correction and are not used as evidence for the current implementation |
