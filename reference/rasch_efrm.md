...
is small – about a third of one per cent on the unit ratio at eight
dichotomous items per set, decaying with set length – and intervals
remain calibrated to at least five thousand linking persons at fine
Monte Carlo resolution; only near ten thousand does the offset approach
the sampling error, and there longer or polytomous sets, not more
persons, buy further accuracy. With `se_method = "bootstrap"`, the
