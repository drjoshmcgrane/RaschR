...
Standard errors use the Godambe sandwich covariance. Inference is
withheld if the empirical sampling-unit score covariance does not
identify every fitted direction. When `judge` is supplied, the
covariance is clustered by judge and inference is also withheld when
there are fewer than ten judges, fewer than eight effective judges, or
no residual cluster degrees of freedom. A caution is attached when the
effective count is below 9.5 or one judge supplies more than 20 per cent
of the comparisons. The pairwise chi-square remains descriptive when
judges are identified because its row-based chi-square reference does
not model within-judge dependence.
