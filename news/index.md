...
- Simulation recovery now matches EFRM and paired-comparison EFRM sets
  by their item or object membership. Renaming the same sets no longer
  removes their unit and origin recovery summaries. A genuinely
  different partition is refused rather than compared under
  coincidentally matching set labels.
...
- The semiparametric EFRM set link now judges grid truncation from the
  ...
  minimum or maximum score in both sets remain in the likelihood but no
  longer count towards the two per cent edge-mass rule, since the masses
