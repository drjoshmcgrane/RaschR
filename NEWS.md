...
* Scree parallel analysis now draws from the exact Rasch distribution
  conditional on each person's observed score and missingness pattern. Saved
  result and application-project signatures use platform-independent binary
  serialisation; analysis files carrying the earlier text signatures remain
  valid across Windows and Unix line endings.
...
* Fitted-model provenance now includes the complete fit, including person
  ...
  are structurally checked and upgraded when opened; saving them again records
  the new integrity information.
