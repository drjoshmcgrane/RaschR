...
* `dimensionality_test()` documents that its default split, chosen from the
  residuals, inflates the proportion of significant person comparisons
  under unidimensionality: in the package's own simulation the
  residual-component split left 6 to 7% of persons significant where a
  split fixed in advance held 5%, and the binomial verdict flagged between
  a sixth and a half of unidimensional samples, depending on the design.
  The earlier documentation claimed the procedure held an exact null. A new `B` argument calibrates the split
  ...
  5% level. Refactored `residual_pca()` and the fit-bootstrap refit so the
  replicates travel the observed computation. The application, summary
  export and report note the inflation where the default split is used.
