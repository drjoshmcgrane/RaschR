...
supplementary person measure without changing the fitted calibration or
its diagnostics. Anchored estimation is available for equating, and
incomplete linked designs can be fitted when their observed response
structure identifies a common scale.
...
global null, separately for each statistic. The simulation page
generates each supported data structure, with controls for its principal
parameters and planted departures. Simulated data can be downloaded as a
CSV or together with the generating call, true values and explanatory
metadata.
...
The DIF bootstrap is a sensitivity analysis under the fitted global
invariant null. The adjusted residual analysis remains primary.
...
The [vignettes](https://drjoshmcgrane.github.io/rasch/articles/) begin
with the data structures required by each model and cover the Rasch
